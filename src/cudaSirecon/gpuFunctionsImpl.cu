...
__host__ void axialShift(int nx, int ny, int nz, int direction, int nphases,
                       int norders, std::vector<GPUBuffer> *bands, int dKZ)
{
...
#endif
  // Allocate memory for result (have to do this out-of-place)
  std::vector<float *> firstBand(2);
  int xdim = (nx / 2 + 1) * 2;
  for (auto i = firstBand.begin(); i != firstBand.end(); ++i)
  {
    cutilSafeCall(cudaMalloc((void **)&(*i), nz * ny * xdim *
                                                 sizeof(float)));
  }
  cutilSafeCall(cudaMemcpyToSymbol(const_firstBandPtrs, &firstBand[0],
                                   firstBand.size() * sizeof(firstBand[0])));
  ...
  int numBlocksY = (int)ceil((float)ny / nThreadsY);
  dim3 nBlocks(numBlocksX, numBlocksY, 1);
  axialShift_kernel<<<nBlocks, nThreads>>>(norders, nphases, nx, ny, nz, dKZ);
  cutilSafeCall(cudaGetLastError());

  // Release the first band data pointers and swap the result pointers into
  // the bands vector, then remove the 3 and 4 vector indexs which coorespond with
  // the 1b bands
  //for (int i = 1; i < 3; ++i)
  //{
    bands->at(1).resize(0);
    bands->at(1).setPtr((char *)firstBand[0],
                            nz * ny * xdim * sizeof(float), 0);
    bands->at(2).resize(0);
    bands->at(2).setPtr((char *)firstBand[1],
                            nz * ny * xdim * sizeof(float), 0);
  //}
  bands->erase(bands->begin() + 3, bands->begin() + 5);
#ifndef NDEBUG
  for (auto i = bands->begin(); i != bands->end(); ++i)
  {
    assert(i->hasNaNs() == false);
...
__global__ void axialShift_kernel(int norders, int nphases,
                                int nx, int ny, int nz, int dKZ)
{
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  // int xdim = (nx / 2 + 1) * 2;
  ...
  if (x < xdim && y < ny)
  {
    ...
    // {
      ...
      for (int z = 0; z < nz; ++z)
      {
        ...
        for (int j = 1; j < nphases - 2; ++j)
        {
          if(j == 2 || j == 4){
            outBasePtr = const_firstBandPtrs[0];
          } else {
            outBasePtr = const_firstBandPtrs[1];
          }
          ...
          float *outPtr = outBasePtr + zidxout * nxy2;
          const float *imgBasePtr = const_imgPtrs[j];
          const float *imgPtr = imgBasePtr + z * nxy2;
          outPtr[offset] += imgPtr[offset];     
        }
                     ...
                     ReconParams *pParams)
{
  ...
  makeoverlaps(bands, overlap0, overlap1, nx, ny, nz, fitorder1, fitorder2,
               (*k0).x, (*k0).y, dxy, dz, OTF, wave, pParams, norders);
  ...
  if (old_k0.x / dkx < (*k0).x - nx / 2)
    (*k0).x -= nx;
  if (old_k0.x / dkx > (*k0).x + nx / 2)
    (*k0).x += nx;
  if (old_k0.y / dky < (*k0).y - ny / 2)
    (*k0).y -= ny;
  if (old_k0.y / dky > (*k0).y + ny / 2)
    (*k0).y += ny;
...
__host__ void makeoverlaps(std::vector<GPUBuffer> *bands,
                           ...
                           std::vector<GPUBuffer> *OTF, short wave, ReconParams *params, int norders)
{
  ...
  else if (params->bBessel)
  {
    ...
    halfangle = acosf(k0mag * order2 / (norders - 1) / kzExMax);
  ...
  // For some reason this does not like an OTF based off of 7 phases.
  //if(!params->bmfSIM){
    ...
    cutilSafeCall(cudaMemcpyToSymbol(const_otfPtrs,
                                    &otfPtrs[0], norders * sizeof(cuFloatComplex *),
                              ...
                              short wave, cuFloatComplex amps[], ReconParams *pParams)
{
  ...
  float amp2 = getmodamp(k0angle, k0mag, bands, overlap0, overlap1, nx, ny, nz,
                         fitorder1, fitorder2, dxy, dz, otf, wave, &modamp, redoarrays, pParams, 0, norders);
  ...
  amp3 = getmodamp(angle, mag, bands, overlap0, overlap1, nx, ny, nz,
                   fitorder1, fitorder2, dxy, dz, otf, wave, &modamp, redoarrays, pParams, 1, norders);
  ...
  if (nz == 1)
  {
    for (int order = 2; order < norders; ++order)
    {
      ...
      getmodamp(angle, mag, bands, overlap0, overlap1, nx, ny, nz,
                order - 1, order, dxy, dz, otf, wave, &modamp, redoarrays, pParams, 1, norders);
      amps[order] = modamp;
    }
  }
  else
  {
    ...
    for (int order = 1; order < norders; ++order)
    {
      ...
      {
        getmodamp(angle, mag, bands, overlap0, overlap1, nx, ny, nz,
                  0, order, dxy, dz, otf, wave, &modamp, redoarrays, pParams, 1, norders);
        amps[order] = modamp;
      }
    }
  }
                         ...
                         int redoarrays, ReconParams *pParams, int bShowDetail, int norders)
{
  ...
  corr_coef = findrealspacemodamp(bands, overlap0, overlap1, nx, ny, nz, order1, order2,
                                  k1, dxy, dz, otf, wave, modamp, &amp_inv, &amp_combo, redoarrays, pParams, norders);
    ...
    ReconParams *pParams, int norders)
{
  ...
  float tan2beta = 2.0f * sqrt(XStarYFR.x * XStarYFR.x + XStarYFR.y * XStarYFR.y) / (sumXMagFR - sumYMagFR);
  ...
  float corr_coef = (XStarYFR.x * XStarYFR.x + XStarYFR.y * XStarYFR.y) / (sumXMagFR * sumYMagFR);
    ...
    cuFloatComplex *XStarY, float *sumXMag, float *sumYMag)
{
  ...
  float angle = 2.0f * M_PI * (((float)i - 0.5f * (float)nx) * k0x + ((float)j - 0.5f * (float)ny) * k0y) * dxy;
                          ...
                          short wave, ReconParams *pParams)
{
  ...
  lambdaexc = 0.88 * lambdaem;
  ;                                                                     /* 0.88 approximates a typical lambdaexc/lambdaem  */
  ...
  {
    if (zdistcutoff[order] >= nz / 2)
      zdistcutoff[order] = ((nz / 2 - 1) > 0 ? (nz / 2 - 1) : 0);
  ...
  for (order = 0; order < norders; order++)
  {
    ...
    cudaMemcpy(dev_scale_debug.getPtr(), dev_scale, nx * ny * nz * sizeof(cuFloatComplex), cudaMemcpyDeviceToHost);
                                    ...
                                    cuFloatComplex *dev_bandptr, cuFloatComplex *dev_bandptr2, bool bSecondEntry)
{
  ...
  if (x1 <= nx / 2)
  {
    ...
    if (order == 0 && conj)
      return; /* For center band only the actual pixels need to be filtered */
    ...
    if (rdist1 <= rdistcutoff)
    {                                             /* is x1,y1 within the theoretical lateral OTF support of
      ...
      for (dir2 = 0; dir2 < ndirs; dir2++)
      {
        for (order2 = -(norders - 1); order2 < norders; order2++)
        {
          if (dir2 == dir && order2 == order)
            continue;
          if (!const_pParams_bFilteroverlaps && !(order2 == 0 && order == 0))
            continue; /* bFilteroverlaps is always true except when (during debug) generating an unfiltered exploded view */
          ...
          if (rdist2 < rdistcutoff)
          {
            ...
            if (const_pParams_bNoKz0 && order2 == 0 && z0 == 0)
              weight = 0.0f;
                                     ...
                                     float dxy, float zoomfact, int z_zoom, float expfact)
{
  ...
  cuFFTErr = cufftExecC2C(myGPUPlan,
                          ...
                          CUFFT_INVERSE);
  if (cuFFTErr != CUFFT_SUCCESS)
    printf("Error in cufftExecC2C: %d\n", cuFFTErr);
                            ...
                            cuFloatComplex *outarray, int nx, int ny, int nz, float zoomfact, int z_zoom)
{
  ...
  xdim = rint(zoomfact * nx);
  ydim = rint(zoomfact * ny);
  zdim = z_zoom * nz;
  nxy = (nx / 2 + 1) * ny;
  nxyout = xdim * ydim;
  ...
  if (x < nx)
  {
    ...
    if (x < 0)
    { /* now xyz get turned into coords of the (half fourier space) input arrays */
      x = -x;
      y = -y;
      z = -z;
      conj = 1;
